...
//    }
  ...
  private dynamic func showActions() {
    ...
    fileprivate func downloadPhoto() {
        Alamofire.request(Five100px.Router.photoDetail(self.photoID, Five100px.ImageSize.large)).validate().responseObject { (photoInfoResp:DataResponse<PhotoInfo>) in
            guard let photoInfo = photoInfoResp.result.value else {
                return
            }
            let downloadImgUrl = photoInfo.url
            let destination = DownloadRequest.suggestedDownloadDestination(for: .documentDirectory, in: .userDomainMask)
            let  progressView = UIProgressView(frame: CGRect(x: 0, y: 66, width: UIScreen.main.bounds.size.width, height: 2))
            self.view.addSubview(progressView)
            let _ = Alamofire.download(downloadImgUrl, to: destination).downloadProgress { progress in
...
extension PhotoViewerViewController {
    fileprivate func loadPhoto() {
        Alamofire.request(Five100px.Router.photoDetail(self.photoID, Five100px.ImageSize.large)).validate().responseObject { (response: DataResponse<PhotoInfo>) in
            guard let photoInfo = response.result.value else {
                return;
            }
            ...
            Alamofire.request(photoInfo.url).responseImage(completionHandler: { (responseImage) in
